...
TEST_CASE("BasicFunctionality", "[Core.BufferedOStream]") {
  ...
  SECTION("WriteSmallData") {
    ...
    for(size_t i = 0; i < stream.buffer_current(); ++i) {
      if(i >= data.size() || i >= stream.buffer_max_size()) { PANIC("Buffer overrun??"); }
      if((char)buffer_data[i] != data[i]) { PANIC("Buffer contents mismatch"); }
    }
  }
  ...
  SECTION("BufferOverflow") {
    ...
    for(size_t i = 0; i < stream.buffer_current(); ++i) {
      if(i >= overflow_data.size() || i >= stream.buffer_max_size()) { PANIC("Buffer overrun??"); }
      if((char)buffer_data[i] != overflow_data[i]) { PANIC("Buffer contents mismatch"); }
    }
  }
  ...
  SECTION("OperatorOverloads") {
    ...
    for(size_t i = 0; i < stream.buffer_current(); ++i) {
      if(i >= expected.size() || i >= stream.buffer_max_size()) { PANIC("Buffer overrun??"); }
      if((char)buffer_data[i] != expected[i]) { PANIC("Buffer contents mismatch"); }
    }
  }
