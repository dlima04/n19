...
inline auto n19::ShitLexer::_token_octal_literal() -> Token {
  ...
  token.pos_   = start;
  token.line_  = line_;
  token.type_  = TokenType::OctalLiteral;
  token.cat_   = TokenCategory::Literal;
  token.value_ = {&src_->at(start), index_ - start};
  return token;
}
...
inline auto n19::ShitLexer::_token_numeric_literal() -> Token {
  ...
  if(std::ranges::find_if(token_raw, [](const char c) {
    ...
    token.pos_   = start;
    token.line_  = line_;
    token.type_  = TokenType::IntLiteral;
    token.cat_   = TokenCategory::Literal;
    token.value_ = token_raw;
    return token;
  }
  ...
  token.pos_   = start;
  token.line_  = line_;
  token.type_  = TokenType::FloatLiteral;
  token.cat_   = TokenCategory::Literal;
  token.value_ = token_raw;
  return token;
}
...
auto n19::ShitLexer::_advance_impl() -> void {
  switch(_current_char()) {
    ...
    case '\b': _token_skip(); return;
    case '\t': _token_skip(); return;
    case '\n': _token_newline(); return;
    case '-':  _token_hyphen(); return;
    case '+':  _token_plus(); return;
    case '*':  _token_asterisk(); return;
    case '/':  _token_fwdslash(); return;
    case '%':  _token_percent(); return;
    case '=':  _token_equals(); return;
    case '<':  _token_lessthan(); return;
    case '>':  _token_greaterthan(); return;
    case '&':  _token_ampersand(); return;
    case '|':  _token_verticalline(); return;
    case '!':  _token_bang(); return;
    case '^':  _token_uparrow(); return;
    case '\'': _token_singlequote(); return;
    case '"':  _token_quote(); return;
    case '`':  _token_quote(); return;
    case ':':  _token_colon(); return;
    case '.':  _token_dot(); return;
    case '\0': _token_null(); return;
    case '~':  _token_tilde(); return;
    case '@':  _token_at(); return;
    case '$':  _token_money(); return;
    case '[':  _token_lsqbracket(); return;
    case ']':  _token_rsqbracket(); return;
    case ';':  _token_semicolon(); return;
    case '(':  _token_lparen(); return;
    case ')':  _token_rparen(); return;
    case '{':  _token_lbrace(); return;
    case '}':  _token_rbrace(); return;
    case ',':  _token_comma(); return;
    case '#':  _token_illegal(); return;
    case '\\': _token_illegal(); return;
    case '?':  _token_illegal(); return;
    default:   _token_ambiguous(); break;
  }
}
